...
- יש מתג ערכת נושא כהה/בהיר בכותרת: שני כפתורי אייקון (שמש/ירח, `data-set-theme="light"`/`"dark"`) עם מסילת החלקה ביניהם (`data-toggle-theme`) שגם ניתנת ללחיצה כמתג רגיל. הלחיצה על אייקון תמיד קובעת את המצב שהוא מייצג (לא מתחלפת הלוך ושוב); הלחיצה על המסילה עצמה מהפכת בין המצבים. זה תלוי בשני דברים שחייבים להישאר בכל קובץ גיליון חדש: התג `<script src="../assets/theme.js"></script>` בתחילת ה־`<head>` (לפני שהעמוד מצטייר, כדי למנוע הבהוב), וכל מבנה ה־`.theme-toggle` בתוך ה־`<header>` (שני הכפתורים + המסילה). שניהם כבר קיימים אם מעתיקים מגיליון קודם — רק לוודא שלא נמחקים בטעות.
- ערכת הבהיר ("מצב שרטוט") מוגדרת ב־`assets/style.css` תחת `html[data-theme="light"]` — כל הצבעים (`--bg`, `--surface`, `--border`, `--text`, `--muted`, ושלושת הניאונים) מוגדרים שם מחדש, אז כל רכיב שכבר משתמש במשתנים האלה מתאים את עצמו אוטומטית. לא צריך לגעת בזה מדי שבוע.
- הלוגו (`ai_bina_logo.png`) נשאר תמונה שקופה רגילה בלי טיפול מיוחד לפי ערכת נושא — בכוונה, כדי לא להוסיף רקע צבעוני מאחוריו.
